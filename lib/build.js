...
async function build({ files, entrypoint, workPath, config = {} }) {
  ...
  // Read nuxt.config.js
  const nuxtConfigFile = require(path.resolve(rootDir, nuxtConfigName));

  // read options from nuxt.config.js otherwise set sensible defaults
  const staticDir = 'static'
  const publicPath = (nuxtConfigFile.build && nuxtConfigFile.build.publicPath) ? nuxtConfigFile.build.publicPath : '_nuxt' // should NOT have leading slash
  const buildDir = nuxtConfigFile.buildDir ? nuxtConfigFile.buildDir : '.nuxt'
  const lambdaName = nuxtConfigFile.lambdaName ? nuxtConfigFile.lambdaName : 'index' // usually just want index, unless deploying 2 nuxt apps
