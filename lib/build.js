...
async function build({ files, entrypoint, workPath, config = {} }) {
  ...
  if (await fs.exists('node_modules_dev')) {
    consola.log('Cache detected for `node_modules` (dev) from previous build...')
  }
  ...
  if (isYarn) {
    await exec('yarn', [ 'install', '--prefer-offline', '--frozen-lockfile', '--non-interactive', '--production=false', '--verbose' ], { env: { NODE_ENV: 'development' } })
  } else {
