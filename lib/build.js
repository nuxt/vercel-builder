...
async function build({ files, entrypoint, workPath, config = {} }) {
  ...
  if (isYarn && !await fs.exists('.yarnclean')) {
    const contents = await fs.readFile(path.join(__dirname, '.yarnclean'))
    await fs.writeFile('.yarnclean', contents, 'utf8')
  }
  ...
  // Static files
  const staticFiles = await globAndPrefix('**', path.join(rootDir, staticDir), 'static')
