...
async function build({ files, entrypoint, workPath, config = {} }) {
  ...
  // Use rootDir/.yarn-mirror for yarn cache
  const yarnMirrorDir = path.join(rootDir, '.yarn-mirror')

  // Check if cache exists
  if (isYarn && await fs.exists(yarnMirrorDir)) {
    consola.info('`.yarn-mirror` detected from previous build')
  }
  ...
  if (isYarn) {
    await exec('yarn', ['config', 'set', 'yarn-offline-mirror', path.join(yarnMirrorDir, 'dev') ])
    await exec('yarn', [ 'install', '--prefer-offline', '--frozen-lockfile', '--non-interactive', '--production=false' ], { env: { NODE_ENV: 'development' } })
  } else {
  ...
  await exec('nuxt', [ 'build', '--standalone', `--config-file "${nuxtConfigName}"` ])
  ...
  if (isYarn) {
    await exec('yarn', [ 'config', 'set', 'yarn-offline-mirror', path.join(yarnMirrorDir, 'prod') ])
    await exec('yarn', [ 'install', '--prefer-offline', '--pure-lockfile', '--non-interactive', '--production=true' ], { env: { NODE_ENV: 'production' } })
  } else {
